...
class Perceptron:
    ...
    def __init__(self, mlp, layer, num_inputs=0, activationFunction='sigmoid', isInputPerceptron=False):
        ...
        self.value = 0
        self.da = {'sigmoid': SigmoidActivationFunction, 'relu': ReLUActivationFunction, 'linear': LinearActivationFunction}
    ...
    def backprop(self, precedingLayer, succeedingLayer, index, learningRate):
        derivative = self.activationFunction.getDerivative()
        if precedingLayer is None:
            return

        if succeedingLayer is None:
            #output layer perceptron
            delta = self.mlp.getLossFunction().getDerivative(index)
        else:
            #hidden layers 
            delta = 0
            for i in range(succeedingLayer.numberOfPerceptrons()):
                delta += succeedingLayer.getPerceptron(i).getDerivative(index)

        self.bias -= learningRate * delta * derivative
        #update weight      
        for i in range(self.num_inputs):
            self.weights[i] -= learningRate * delta * derivative * precedingLayer.getPerceptron(i).getValue()
        
        # update derivivates of perceptron
        self.derivatives *= delta
...
class MultiLayerPerceptron:
    ...
    def learn(self,inputs, outputs, learningRate, numberOfEpochs):
        ...
        if (inputs.shape[1] != self.layers[0].numberOfPerceptrons() or outputs.shape[1] != self.layers[self.numberOfLayers()-1].numberOfPerceptrons() or inputs.shape[0] != outputs.shape[0]):
            ...
            for i in range(numberOfEpochs):
                ...
                for k in range(inputs.shape[0]):
                    ...
                    # print(self.getLossFunction().getValue())
